...
TRY("r19_dynamic_logo_srcset",()=>{const apply=img=>{if(!img||img.srcset)return;const src=img.getAttribute("src")||"",m=src.match(/(?:^|\/)IMG\/logos\/(.+)-(nsq|hz)-(160|384)\.webp$/);if(!m)return;const name=m[1],kind=m[2];if(kind==="nsq"){img.srcset=`${W.__EXP_BASE__||"/"}IMG/logos/${name}-nsq-160.webp 160w, ${W.__EXP_BASE__||"/"}IMG/logos/${name}-nsq-320.webp 320w`;img.sizes=img.closest(".sys-icon")?"90px":"92px";img.width=160;img.height=160}else{const dimensions={"anticipa":[384,124],"bancos":[384,96],"comercial-premium":[384,129],"comercial-pro":[384,120],"comercial-start":[384,131],"contabilidad":[384,111],"contabilidad-nube":[384,109],"evalua":[384,136],"factura":[384,104],"nominas":[384,86],"nominas-nube":[384,126],"optimiza":[384,112],"vende":[384,100],"xml":[384,131]},dim=dimensions[name]||[384,128],top=name==="comercial-start"?740:name==="comercial-pro"?738:768;img.srcset=`${W.__EXP_BASE__||"/"}IMG/logos/${name}-hz-384.webp 384w, ${W.__EXP_BASE__||"/"}IMG/logos/${name}-hz-${top}.webp ${top}w`+(name==="nominas"?`, ${W.__EXP_BASE__||"/"}IMG/logos/nominas-hz-1440.webp 1440w`:"");img.sizes=img.classList.contains("calc-syslogo")?"260px":img.classList.contains("sum-logo")?"120px":"240px";img.width=dim[0];img.height=dim[1]}img.dataset.r19Logo="dynamic-"+kind};const scan=root=>(root.matches?.('img[src*="IMG/logos/"]')&&apply(root),root.querySelectorAll?.('img[src*="IMG/logos/"]').forEach(apply));scan(D);new MutationObserver(ms=>ms.forEach(m=>m.addedNodes.forEach(n=>n.nodeType===1&&scan(n)))).observe(D.documentElement,{childList:true,subtree:true})});
