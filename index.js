...
async function loadPartial(placeholderId,fileName){const ph=document.getElementById(placeholderId);if(!ph)return;const cands=[prefix(`PARTIALS/${fileName}`),isGh&&repoBase?`${repoBase}/PARTIALS/${fileName}`:null,!isGh?`${depth}PARTIALS/${fileName}`:null,"/expiriti-r19-preview-20260826/PARTIALS/"+fileName].filter(Boolean);let html="",lastErr=null;for(const u of cands)try{const url=u+(u.includes("?")?"&":"?")+"v=2026.08-r19-human8";const resp=await fetch(url,{cache:DEBUG_NOCACHE?"reload":"force-cache"});if(!resp.ok)throw new Error("HTTP "+resp.status+" "+resp.statusText);html=await resp.text();break}catch(e){lastErr=e}if(!html){console.warn("[Expiriti] No se pudo cargar partial:",fileName,lastErr);return}const template=document.createElement("template");template.innerHTML=html;ph.replaceWith(template.content)}
...
try{
  const url="/expiriti-r19-preview-20260826/data/promociones.json?d="+(new Date).toISOString().slice(0,10);
